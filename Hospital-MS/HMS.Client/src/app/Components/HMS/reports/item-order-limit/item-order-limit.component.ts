...
export class ItemOrderLimitComponent implements OnInit {
  ...
  showReport() {
    ...
    const { StoreId } = this.itemOrderLimitForm.value;
      ...
      if (response.isSuccess) {
        ...
        response.results.forEach((mainGroup: any) => {
          mainGroup.itemGroups.forEach((itemGroup: any) => {
            if (!groupedData[itemGroup.itemGroupId]) {
              groupedData[itemGroup.itemGroupId] = {
            ...
            itemGroup.items.forEach((item: any) => {
              ...
              groupedData[itemGroup.itemGroupId].items.push(item);
            });
          });
        });
  ...
  printReport() {
    ...
    const options = {
      margin:       0.5,
      filename:     `تقرير حد الطلب - ${this.selectedStoreName}.pdf`,
