...
import {
  FilterModel,
  PagingFilterModel,
} from '../../../../Models/Generics/PagingFilterModel';
...
export class MedicalServicesListComponent implements OnInit {
  ...
  openServiceDetails(serviceId: number) {
    this.appointmentService.getServices().subscribe({
      next: (data) => {
        this.serviceDetails = data.results.find(
        ...
        this.serviceForm.patchValue({
          ...
          weekDays: this.serviceDetails.medicalServiceSchedules.map(
            (schedule: any) => ({ weekDay: schedule.weekDay })
          ),
  ...
  editService() {
    ...
    this.appointmentService
      .editService(this.serviceDetails.id, payload)
      .subscribe({
        next: (data) => {
          this.getServices();
          this.messageService.add({
        ...
        error: (err) => {
          this.messageService.add({
            severity: 'error',
            summary: 'حدث خطأ',
            detail: 'حدث خطأ أثناء تعديل الخدمة',
          });
        },
      });
  }
